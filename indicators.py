...
def price_sma_ratio(prices, n):
    ...
    price_sma = (price_sma - price_sma.mean()) / \
        price_sma.std()  # normalizing the indicator
    return price_sma
...
def bollinger_bands(prices, n):
    ...
    # replacing inf with NaN and dropping rows with NaN's
    bb = bb.replace([np.inf, -np.inf], np.nan).dropna()
