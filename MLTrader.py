...
class MLTrader:
    ...
    def fit(self, symbol="IBM", sd="2008-01-01", ed="2009-01-01"):
        ...
        prices_norm = pd.DataFrame(self.ss.fit_transform(
            prices), index=prices.index, columns=[symbol])
        # generating indicator dataframe for predicting
        features_df = self.generate_indicators(
            prices_norm).fillna(method='bfill')
    ...
    def save_learner(self, symbol=""):
        ...
        dump(self.learner, os.path.join(
            folder_path, "{}_model.joblib".format(symbol)))
    ...
    def testLearner(self, symbol="IBM", sd="2009-01-01", ed="2010-01-01"):
        ...
        for i in range(self.n, trades_df.shape[0]-1):
            ...
            # price goes up
            if self.prices_pred.iloc[i+1, 0] > prices_norm.iloc[i, 0]*(1+self.impact):
                trades_df.iloc[i, 0] = 1000 - \
                    current_holdings  # buy as much as we can
                ...
                if prices_norm.iloc[i+1, 0] < prices_norm.iloc[i, 0]:
                    self.bad_trades += 1
            # price goes down
            elif self.prices_pred.iloc[i+1, 0] < prices_norm.iloc[i, 0]*(1-self.impact):
                trades_df.iloc[i, 0] = -1000 - \
                    current_holdings  # sell as much as we can
